...
contract AdminFunctions 
{
    ...
    address  _nullAddr;
    uint256 transactionIdConstant=1223132;
    ...
    */
    ...
    /**
     * Only Admin can topup USDT for "Registered user" (for 120  USD put 120 as value)
     * 
    **/
    ...
    **/
     ...
     * 
    **/
    function addTransaction(uint256 qty,uint256 price,uint256 totalVal,string name)  internal  returns(uint256 transactionId)
    {
        Transaction memory _transaction;
        _transaction.transactionId=transactionIdConstant.add(1);
        _transaction.productNam=name;
        _transaction.price=price;
        _transaction.quantity=qty;
        _transaction.dateOfPurchase=now;
        _transaction.totalVal=totalVal;
        transactionIdConstant=transactionIdConstant.add(1);
...
contract ABCStore is AdminFunctions
{
    ...
    **/
     ...
     * 4) Is item in stock?
     * 5) Give 10% discount to elite members
     * 
        ...
        }else
        {
            ...
            if(totalVal<=checkUsdtBalance(msg.sender))
            {
                ...
                if(purchaseSkirt(_userList[msg.sender]) ){
                    ...
                    burnTokens(msg.sender,totalVal);
                    ...
                    transactionID=addTransaction(qty,_productList[stringToBytes32(item)].price,totalVal,_productList[stringToBytes32(item)].name);
